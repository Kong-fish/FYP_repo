...
import "../AdminDashboard.css"; // For global theme variables and .admin-smt-container
...
const ReviewLoan: React.FC = () => {
    ...
    const getPrediction = async (loanApplicationData: LoanApplication) => {
        // Implement your LLM call here, similar to the pattern below
    ...
    const handleGetPrediction = async () => {
        ...
        // The PredictionResult modal's internal isLoadingPrediction state will handle its own loader.
        // We just need to pass the current loanDetails.ai_prediction if it exists, or update it
        // after fetching a new one.

        // If you want to force a new prediction every time the button is clicked,
        // uncomment the following block and ensure your `getPrediction` function
        // integrates with your actual backend/LLM.
        /*
        try {
            // Simulate fetching new prediction data (replace with actual fetch to your LLM API)
            const newPrediction = await getPrediction(loanDetails); // Your function to call LLM
            setLoanDetails(prev => prev ? { ...prev, ai_prediction: newPrediction } : null);
        } catch (err) {
            console.error("Error getting prediction:", err);
            alert("Failed to get new prediction."); // Or show message in modal
        }
        */
    };
    ...
    return (
        <div className="admin-smt-container">
                    ...
                    <div className="admin-smt-card-header mb-6">
                        ...
                        <div className="flex gap-2">
                            ...
                            <button
                                ...
                                className="admin-smt-btn admin-smt-btn-primary" // Apply primary button style
                                disabled={loading || !loanDetails} // Disable if loan details are loading or not available
                            >
                                Get Prediction
                            </button>
                        </div>
            ...
            <PredictionResult
                ...
                isLoadingPrediction={false} // This modal handles its own loading for feature importances
