...
interface FormData {
  ...
  customer_home_ownership: string; // This will now be selected from a dropdown
  customer_job_company_name: string;
  customer_job_title: string;
  customer_job_years: string;
  loan_intent: string; // This will now be selected from a dropdown
  loan_amount: string;
...
export default function Cust_Apply_Loan() {
  ...
  const [formData, setFormData] = useState<FormData>({
    ...
    customer_home_ownership: '', // Initialize as empty string
    customer_job_company_name: '',
    customer_job_title: '',
    customer_job_years: '',
    loan_intent: '', // Initialize as empty string
    loan_amount: '',
    account_number: '',
  ...
  const validate = (): Errors => {
    ...
    // Updated validation for dropdowns
    ...
    else if (formData.loan_intent === "") newErrors.loan_intent = "Please select a loan purpose.";
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      // 1. Get the current logged-in user (auth.users.id)
      ...
      // --- CRUCIAL CHANGE: Fetch customer_id from public.Customer using user.id (which is user_uuid) ---
      const { data: customerProfile, error: customerProfileError } = await supabase
        .from('Customer')
        .select('customer_id') // We only need customer_id for the Loan table FK
        .eq('user_uuid', user.id) // Link auth.users.id to Customer.user_uuid
        .single();
      ...
      if (customerProfileError || !customerProfile) {
        ...
        if (customerProfileError && customerProfileError.code !== 'PGRST116') {
          ...
          setSubmitError("Failed to retrieve customer profile data. Please try again or contact support.");
        } else {
          // No customer profile found for this user_uuid
          // This typically means the user hasn't completed their profile (first_name, last_name, etc.)
          setSubmitError("Your customer profile is incomplete or missing. Please ensure your profile details (e.g., First Name, Last Name) are filled out on your dashboard before applying for a loan.");
        }
        setIsSubmitting(false);
        return;
      }

      // Now we have the actual customer_id from the public.Customer table
      const actualCustomerId = customerProfile.customer_id;

      // 2. Prepare data for Supabase insertion into 'Loan' table
      const dataToInsert = {
        customer_id: actualCustomerId, // Use the fetched customer_id from the Customer table
        ...
        // application_date, loan_id, loan_grade, customer_default, loan_interest_rate are handled by Supabase defaults
        // ai_prediction, final_approval: These might be updated later by an admin or AI process
      };
      ...
      if (error) {
        ...
        // Provide a more user-friendly error message based on error details
        if (error.code === '23503' && error.details?.includes('Loan_account_number_fkey')) {
        ...
        navigate('/customer-loan-complete'); // Redirect on success
      }
    } catch (error) {
                ...
                <div className="form-section-card">
                  ...
                  <div className="form-row">
                    <div className="form-group">
                      ...
                      >
                        <option value="">Select...</option>
                        {/* Options based on common loan intents, you might adjust this based on your public.loan_intent enum */}
